...
## Hotel collateral (in progress)

Groundwork for merging property data into existing InDesign templates for
The Ritz-Carlton, Grand Cayman: an in-room directory, a restaurant /
room-service menu, a rate / room-type sheet, and an event / promo flyer.

- `schemas/` holds JSON Schemas for each collateral type.
- `examples/` holds placeholder data that matches them.
- [`docs/PIPELINE.md`](docs/PIPELINE.md) explains how InDesign's CSV-driven
  data merge works and how each JSON shape flattens into the CSV rows that
  `POST /api/merge` consumes.

No flatten-to-CSV script exists yet: the CSV column names depend on a real
template's frame mapping, so that waits on an actual INDD template.
